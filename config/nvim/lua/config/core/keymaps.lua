...
keymap.set("n", "<C-s>", function()
    ...
    if ft == "c" or ft == "cpp" then
        _G._42tools.norminette.format_file()
    elseif ft == "python" then
        _G._42tools.flake8.format_file()
    else
        vim.cmd("write")
  end
end, { desc = "Smart format by filetype", silent = true })
...
vim.defer_fn(function()
    vim.notify(
        "\n" ..
        "  ✓ Formatters loaded:\n" ..
        "  Ctrl+S:     format (c_formatter_42 / autopep8)\n" ..
        "  <leader>n:  check file (norminette / flake8)\n" ..
        "  <leader>N:  check dir  (norminette / flake8)",
        vim.log.levels.INFO,
        {
            title = "Formatter: 42 Norminette & Flake8",
            timeout = 3000,
        }
    )
end, 200)
